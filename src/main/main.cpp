/*
        RDPSOVina version 2.0

        This file is revised from main.cpp in AutoDock Vina.

        Authors: Giotto H. K. Tai  <marcus.ckng@gmail.com>
...
#include "parallel_rdpso.h" //RDPSOVina
#include "file.h"
#include "cache.h"
#include "non_cache.h"
...
std::string vina_remark(fl e, fl lb, fl ub) {
	...
	remark << "REMARK VINA RESULT: " 
		                << std::setw(9) << std::setprecision(9) << e
...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
			   ...
			   const parallel_pso& par/*RDPSOVina*/, fl energy_range, sz num_modes, 
			   int seed, int verbosity, bool score_only, bool local_only, tee& log, const terms& t, const flv& weights,int num_birds, double c1, double c2) {
	...
	if(score_only) {
		...
		refine_structure(m, prec, nc, out, authentic_v, par.pso.ssd_par.evals/*RDPSOVina*/);
		done(verbosity, log);
		...
		par(m, out_cont, prec, ig, prec_widened, ig_widened, corner1, corner2, generator, num_birds, c1, c2);
		done(verbosity, log);
...
if(!out_cont.empty()) {
		...
		VINA_FOR_IN(i, out_cont)
			refine_structure(m, prec, nc, out_cont[i], authentic_v, par.pso.ssd_par.evals/*RDPSOVina*/);
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
				 ...
				 const flv& weights,
				 int cpu, int seed, int verbosity, sz num_modes, fl energy_range, tee& log, int num_birds,double ratio_steps,double c1,double c2) {
	...
	parallel_pso par;/*RDPSOVina*/
	sz heuristic = m.num_movable_atoms() + 10 * m.get_size().num_degrees_of_freedom();
	par.pso.num_steps = unsigned(70 * 3 * (50 + heuristic) / 2 * ratio_steps); // 2 * 70 -> 8 * 20 // FIXME/*RDPSOVina*/
	par.pso.ssd_par.evals = unsigned((25 + m.num_movable_atoms()) / 3);/*RDPSOVina*/
	par.pso.min_rmsd = 1.0;/*RDPSOVina*/
	par.pso.num_saved_mins = 20;/*RDPSOVina*/
	par.pso.hunt_cap = vec(10, 10, 10);/*RDPSOVina*/
	...
	if(randomize_only) {
		...
		if(no_cache) {
			do_search(m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
					  ...
					  seed, verbosity, score_only, local_only, log, t, weights,num_birds,c1,c2);
		}
		else {
...
int main(int argc, char* argv[]) {
	...
	const std::string version_string = "RDPSOVina version 2.0";    //pso
	const std::string error_message = "\n\n\
Please contact author, Giotto, H. K. Tai <giottotai@yahoo.com.hk> or Shirley, W. I. Siu <shirleysiu@umac.mo>, so\n\
...
Visit http://cbbio.cis.umac.mo for more information.                   \n\
                                                                       \n\
RDPSOVina was developed based on the framework of AutoDock Vina.         \n\
                                                                       \n\
...
=======================================================================\n";
	...
	try {
		...
		double ratio_steps=0.06,c1=2.2250738585072014e-308,c2=2.2250738585072014e-308;
		...
		;
		options_description pso("RDPSO parameters (optional)");
		pso.add_options()
			("num_particles", value<int>(&num_birds)->default_value(12), "Number of particles per thread")
			("ratio_steps", value<double>(&ratio_steps)->default_value(0.06,"0.06"), "Ratio of actual iterative steps to total steps")
			("c1", value<double>(&c1)->default_value(1.0,"1.0"), "Cognitive weight")
			("c2", value<double>(&c2)->default_value(1.0,"1.0"), "Social weight")
		;
		...
		main_procedure(m, ref, 
					...
					gd, exhaustiveness,
					weights,
					cpu, seed, verbosity, max_modes_sz, energy_range, log, num_birds,ratio_steps,c1,c2);
	}
	catch(file_error& e) {
